...
# Function to read and tidy a single year sheet for inmate-on-inmate abusive sexual contact, alleged
read_and_tidy_year_sheet <- function(sheet_name) {
  ...
  # Based on the structure, we want "Inmate-on-inmate" -> "Alleged" column (abusive sexual contact, alleged)
  ...
  # Find the "Alleged" row (should be row 2 based on structure)
  ...
  if (length(alleged_row) == 0) {
    cat("No 'Alleged' row found in", sheet_name, "\n")
    return(data.frame(state = character(), year = integer(), subtype = character(), value = double()))
  }
  ...
  # Get the inmate-on-inmate alleged values (column 6 based on structure)
  # Based on structure: column 6 contains the alleged values for abusive sexual contact
  ...
  # Extract values from the inmate-on-inmate alleged column (column 6)
  # Based on structure: column 6 contains the alleged values
  alleged_col_idx <- which(names(state_data) == inmate_col) + 2  # Skip to alleged column
  if (alleged_col_idx > ncol(state_data)) {
    cat("Could not find alleged column in", sheet_name, "\n")
    return(data.frame(state = character(), year = integer(), subtype = character(), value = double()))
  }
  
  values <- state_data[[alleged_col_idx]][1:length(states)]
  ...
  result <- data.frame(
    ...
    subtype = "inmate-on-inmate abusive sexual contact, alleged",
    value = values,
...
output_data_path <- file.path(output_data_dir, "panel_prea_abusive_sexual_contact_alleged_2012_2018.csv")
...
# Create plot
if (nrow(prea_long) > 0) {
  plot_df <- prea_long %>% filter(subtype == "inmate-on-inmate abusive sexual contact, alleged")
  
  if (nrow(plot_df) > 0) {
    # Plot: annual trends by state, faceted
    p <- ggplot(plot_df, aes(x = year, y = value, group = state)) +
      geom_line(color = "#FF7F00", linewidth = 0.4, alpha = 0.9) +
      geom_point(color = "#FF7F00", size = 0.8, alpha = 0.9) +
      facet_wrap(~ state, ncol = 8, scales = "free_y") +
      labs(
        title = "Inmate-on-inmate Abusive Sexual Contact (Alleged), 2012–2018",
    ...
    # Save figure
    fig_path <- file.path(output_fig_dir, "annual_inmate_on_inmate_abusive_sexual_contact_alleged_2012_2018.png")
