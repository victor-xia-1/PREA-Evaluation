...
# Paths
data_path <- "/Users/victorxia/Documents/Honors Thesis/data/clean/panel_prea_2012_2018.csv"
...
create_interactive_plot <- function(data) {
  ...
  p <- plot_ly(data, x = ~year, y = ~value, 
    ...
    layout(
      title = list(
        text = "Inmate-on-inmate Nonconsensual Sexual Acts (Alleged), 2012–2018",
        font = list(size = 16)
      ),
      xaxis = list(
        title = "Year",
...
create_heatmap_plot <- function(data) {
  ...
  # Convert to matrix
  state_names <- heatmap_data$state
  year_matrix <- as.matrix(heatmap_data[, -1])
  rownames(year_matrix) <- state_names
  ...
  layout(
    title = list(
      text = "Inmate-on-inmate Nonconsensual Sexual Acts (Alleged) Heatmap, 2012–2018",
      font = list(size = 16)
    ),
    xaxis = list(title = "Year"),
    yaxis = list(title = "State", categoryorder = "array", categoryarray = rev(state_names))
  ) %>%
  config(
    displayModeBar = TRUE,
    modeBarButtonsToRemove = c("pan2d", "lasso2d", "select2d"),
...
create_state_comparison_plot <- function(data) {
  ...
  p <- plot_ly(top_data, x = ~year, y = ~value, 
    ...
    layout(
      title = list(
        text = "Top 10 States: Inmate-on-inmate Nonconsensual Sexual Acts (Alleged), 2012–2018",
        font = list(size = 16)
      ),
      xaxis = list(
        title = "Year",
...
htmlwidgets::saveWidget(p1, file.path(output_fig_dir, "interactive_prea_overlay.html"), selfcontained = FALSE)

# 2. Heatmap
p2 <- create_heatmap_plot(prea_data)
htmlwidgets::saveWidget(p2, file.path(output_fig_dir, "interactive_prea_heatmap.html"), selfcontained = FALSE)

# 3. Top 10 states comparison
p3 <- create_state_comparison_plot(prea_data)
htmlwidgets::saveWidget(p3, file.path(output_fig_dir, "interactive_prea_top10.html"), selfcontained = FALSE)

cat("Interactive visualizations saved:\n")
cat("1. Overlay plot (all states):", file.path(output_fig_dir, "interactive_prea_overlay.html"), "\n")
cat("2. Heatmap:", file.path(output_fig_dir, "interactive_prea_heatmap.html"), "\n")
cat("3. Top 10 states:", file.path(output_fig_dir, "interactive_prea_top10.html"), "\n")
