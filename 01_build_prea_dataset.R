...
# Function to read and tidy a single year sheet
read_and_tidy_year_sheet <- function(sheet_name) {
  ...
  # Based on the structure, we want "Inmate-on-inmate" -> "Alleged" column
  ...
  # States start after the header rows (around row 6 based on structure)
  state_start <- alleged_row + 4  # Skip header rows
  ...
  # Get the inmate-on-inmate alleged values (column 4 based on structure)
  ...
  # Extract values from the inmate-on-inmate column
  values <- state_data[[inmate_col]][1:length(states)]
  ...
  result <- data.frame(
    ...
    subtype = "inmate-on-inmate nonconsensual sexual acts, alleged",
    value = values,
...
output_data_path <- file.path(output_data_dir, "panel_prea_2012_2018.csv")
...
# Create plot
if (nrow(prea_long) > 0) {
  plot_df <- prea_long %>% filter(subtype == "inmate-on-inmate nonconsensual sexual acts, alleged")
  ...
  if (nrow(plot_df) > 0) {
    ...
    p <- ggplot(plot_df, aes(x = year, y = value, group = state)) +
      ...
      labs(
        title = "Inmate-on-inmate nonconsensual sexual acts (alleged), 2012–2018",
    ...
    # Save figure
    fig_path <- file.path(output_fig_dir, "annual_inmate_on_inmate_nonconsensual_sexual_acts_alleged_2012_2018.png")
